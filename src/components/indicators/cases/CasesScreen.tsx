...
import { getViolenceCasesByModalityByYear, getViolenceCasesByViolenceTypeByYear } from '@/lib/fetchers/cases'
import { type TypeViolenceCaseModality, type TypeViolenceCaseType } from '@/types'
import { APP_MESSAGES, VIOLENCE_CONTENT, VIOLENCE_MODALITIES_CONTENT, VIOLENCE_MODALITIES_MAPPER, VIOLENCE_RGB_COLORS } from '@/constants'
     ...
     <>
      ...
      <Suspense key={year + 'violenceType'} fallback={<Skeleton height={539} width={1200} />}>
        <Await promise={dataPromiseViolenceType}>
          {
            (data: TypeViolenceCaseType | undefined) => <GridByPie data={data} label={APP_MESSAGES.TITLE_VIOLENCE_TYPE} backgroundColors={VIOLENCE_RGB_COLORS} />
          }
        </Await>
      </Suspense>
      <div className='p-8'>
